...
void USART3_callback_fn(uint8_t byte)
{
	...
	if( fsm_lock() == FSM_LOCK_ACQUIRED )
	{
		...
		// current packet ready to be buffered
			...
			// TODO: SHOW STEVE HOW I AM ECHOING THE EXACT RAW BYTE I AM RECIEVING
//			pingbuff[j] = byte;
//			j++;
//			if (j== 2) {
//				USART2_putchar('!');
...
//			}
		...
		// current packet ready to be buffered
		else if (current_state == STATE_DMA || current_state == STATE_UPDATE || current_state == STATE_GOT_ID
...
void userbutton_callback_fn(void)
{
	if( fsm_lock() == FSM_LOCK_ACQUIRED )
	{
		...
		// if pinging, go to update
			...
			TIM7_kill();
			fsm_set_state(STATE_DMA); // fsm_set_state(STATE_ECHO_BYTES);
		}

		// if updating, go to ID_update
		else if (current_state == STATE_DMA || current_state == STATE_UPDATE || current_state == STATE_UPDATE_RECV)
...
//		USART2_putstr(": ");
//		USART2_putstr((uint8_t*)int2str(buffer[i]));
//		USART2_putstr("\n\r");
//	}
		uint32_t avg = ADC_printBuffer();
		WIFI_send_update(avg);
		TIM7_2_sec();
...
void TIM7_callback_fn(void)
{
	...
	if( fsm_lock() == FSM_LOCK_ACQUIRED )
	{
		...
		else if (current_state == STATE_DMA)
