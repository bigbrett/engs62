...
void USART3_init(void(*USART3_rx_callback)(uint8_t byte))
{
	...
	/* Configure PD8 as AF7 (USART1-3) and as Output (push-pull) */
	...
	/* Configure PD9 as AF7 (USART1-3) and as Input (reset state) */
	GPIOD->AFRL &= GPIOx_AFRH_AFRH9_MASK;
	GPIOD->AFRL |= GPIOx_AFRH_AFRH9_AF7;
	GPIOD->MODER &= GPIOx_MODER_PIN9_MASK;
	GPIOD->MODER |= GPIOx_MODER_PIN9_AF;

	/* Reset the USART3 peripheral and enable its clock */
