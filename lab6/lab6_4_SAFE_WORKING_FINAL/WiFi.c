...
void WIFI_send_ping()
{
	...
	for (i=0; i<sizeof(Ping_t); i++) {
		...
		// TODO: SHOW STEVE HOW I AM ECHOING EXACTLY WHAT I AM SENDING
		USART2_putchar(*pbyte + 48);
		pbyte++;
	}
	USART2_putstr("\n\rBytes sent\n\r");
...
{
	uint8_t foo = response_ping.type;
	if (response_ping.type == PING) {
		...
		USART2_putstr("\n\rreceived type = ");USART2_putchar(response_ping.type + 48);
